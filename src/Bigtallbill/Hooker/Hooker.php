...
class Hooker
{
    ...
    public function install()
    {
        ...
        file_put_contents($this->projectDir . '/hooker.json', json_encode($config));

        foreach ($this->hooks as $hook) {
            file_put_contents($hookDir . '/' . $hook, $this->createHook($hook));
            chmod($hookDir . '/' . $hook, 0754);
        }

        copy(
            $this->hookerRoot . DIRECTORY_SEPARATOR . 'bin' . DIRECTORY_SEPARATOR . 'hooker.phar',
            $this->projectDir . DIRECTORY_SEPARATOR . 'hooker.phar'
        );

        chmod($this->projectDir . DIRECTORY_SEPARATOR . 'hooker.phar', 0754);
    }
